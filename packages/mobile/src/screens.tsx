/**
 * Cato screens — the four tabs + pair/settings, presentational. State + wiring live in
 * App.tsx. Implements the Cato.dc.html design direction.
 */
...
import { Icon, Dot, StatusDot, Pill, RiskBadge, SectionLabel, Card, Btn, IconChip } from "./ui";
...
export function AppBar({ linked }: { linked: boolean }) {
  return (
    <View style={st.appBar}>
      ...
      <View style={[st.linkPill, { borderColor: tint(linked ? C.active : C.attention, 0.25), backgroundColor: tint(linked ? C.active : C.attention, 0.12) }]}>
        <Dot color={linked ? C.active : C.attention} glow={linked} />
        <Text style={{ color: linked ? C.active : C.attention, fontSize: 12, fontWeight: "600" }}>
          {linked ? "Desktop linked" : "Offline"}
        </Text>
      </View>
    </View>
  );
...
export function ScreenTitle({ title, sub, right }: { title: string; sub?: string; right?: ReactNode }) {
  return (
    <View style={{ paddingHorizontal: S.xl, paddingTop: S.sm, paddingBottom: 14 }}>
...
export function TalkScreen({
  projects, exchange, recording, busy, hint, onPressIn, onPressOut, onOpenProject, onGoApprovals, approvals,
}: {
  ...
  return (
    <View style={{ flex: 1 }}>
      <AppBar linked />
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: S.xl, paddingBottom: 12 }} showsVerticalScrollIndicator={false}>
        <SectionLabel right={
          <Pressable onPress={onGoApprovals} style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
            <Text style={{ color: C.accent, fontSize: 12.5, fontWeight: "500" }}>All {projects.length} projects</Text>
        ...
        <View style={{ gap: 10, marginBottom: 16 }}>
          {needs.length === 0 && (
            <Card style={{ alignItems: "center", paddingVertical: 22 }}>
              ...
              <Text style={{ color: C.textDim, marginTop: 8 }}>Nothing needs you right now.</Text>
            </Card>
          )}
          ...
          {quiet.length > 0 && (
            <View style={st.quietRow}>
              {quiet.slice(0, 3).map((p) => (
                <View key={p.name} style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                  <StatusDot state={p.state as StatusKey} glow={p.state === "active"} />
                  <Text style={{ color: C.textDim, fontSize: 13, fontWeight: "500" }}>{p.name}</Text>
                </View>
              ))}
              <View style={{ flex: 1 }} />
              <Text style={{ color: C.textFaint, fontSize: 11.5 }}>running quietly</Text>
            </View>
          )}
        </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScreenTitle
        ...
        right={approvals.length ? <Pill color={C.waiting}><View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}><Dot color={C.waiting} /><Text style={{ color: C.waiting, fontWeight: "600", fontSize: 12.5 }}>{approvals.length} pending</Text></View></Pill> : undefined}
      />
      {approvals.length === 0 ? (
        <AllClear />
      ) : (
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 14, gap: 12 }} showsVerticalScrollIndicator={false}>
...
export function ApprovalCard({ a, onResolve, onOpen }: { a: ApprovalRequest; onResolve: (id: string, d: "allow" | "deny") => void; onOpen: () => void }) {
  ...
  return (
    <View style={[st.apCard, { borderColor: tint(color, a.risk === "high" ? 0.4 : 0.18) }]}>
      ...
      <View style={{ padding: 14, paddingTop: 13 }}>
        ...
        {diff.length > 0 && (
          <View style={[st.codeBox, { borderColor: tint(color, 0.2) }]}>
            {diff.map((l, i) => {
              ...
              return (
                <View key={i} style={[st.codeLine, add && { backgroundColor: tint(C.active, 0.08) }, del && { backgroundColor: tint(C.attention, 0.08) }]}>
                  <Text style={[st.code, add && { color: C.add }, del && { color: C.del }, a.tool === "Bash" && { color: C.del }]} numberOfLines={1}>
        ...
        <View style={{ flexDirection: "row", gap: 10, marginTop: 13, alignItems: "center" }}>
          {a.risk === "high" ? (
            <>
              ...
              <View key={i} style={st.feedRow}>
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Text style={st.feedSummary} numberOfLines={2}>
                    {e.summary}{e.project ? <Text style={{ color: C.textMute }}>  · {e.project}</Text> : null}
                  </Text>
                </View>
              </View>
            );
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScreenTitle title="Projects" right={
        <Pressable onPress={onStart} style={st.startChip}><Icon name="plus" size={15} color={C.onAccent} /><Text style={{ color: C.onAccent, fontWeight: "600", fontSize: 13 }}>Start</Text></Pressable>
      } />
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 14, gap: 10 }} showsVerticalScrollIndicator={false}>
        {projects.length === 0 && <Text style={{ color: C.textMute, padding: 16 }}>No projects yet. Tap Start to launch an agent.</Text>}
        {projects.map((p) => {
          const color = STATUS[p.state as StatusKey];
          return (
  ...
  return (
    <View style={{ flex: 1, paddingHorizontal: 30, alignItems: "center" }}>
      ...
      <View style={st.pairCard}>
        <View style={[st.apIcon, { backgroundColor: tint(C.accent, 0.12), width: 44, height: 44, borderRadius: 12 }]}><Icon name="desktop" size={22} color={C.accent} /></View>
        <View style={{ flex: 1, minWidth: 0 }}>
          <Text style={{ color: C.text, fontWeight: "600", fontSize: 15 }}>Desktop address</Text>
      ...
      <Pressable onPress={onConnect} style={st.pairBtn}>
        {connecting ? <ActivityIndicator color={C.onAccent} /> : <><Icon name="link" size={17} color={C.onAccent} /><Text style={{ color: C.onAccent, fontWeight: "600", fontSize: 16 }}>Pair with this Mac</Text></>}
      </Pressable>

      <View style={st.relayCard}>
        <View style={[st.apIcon, { backgroundColor: tint(C.accent, 0.12), width: 38, height: 38, borderRadius: 11 }]}><Icon name="globe" size={19} color={C.accent} /></View>
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 7 }}>
            <Text style={{ color: C.text, fontWeight: "600", fontSize: 13.5 }}>Away from home? Cato Relay</Text>
            <Pill bg={C.accent}><Text style={{ color: C.onAccent, fontWeight: "700", fontSize: 9 }}>PRO</Text></Pill>
          </View>
          <Text style={{ color: C.textDim, fontSize: 12, marginTop: 2 }}>Reach your desktop from any network — encrypted.</Text>
        </View>
      </View>
    </View>
  );
    ...
    <View style={st.listenOverlay}>
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 34 }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 24 }}>
          <Dot color={C.accent} />
          <Text style={{ color: C.accent, fontSize: 12, fontWeight: "600", letterSpacing: 1.5 }}>LISTENING</Text>
        </View>
      ...
      <View style={{ alignItems: "center", paddingBottom: 40 }}>
        <View style={[st.pttBtn, st.pttBtnActive, { width: 84, height: 84 }]}><Icon name="mic" size={34} color="#fff" /></View>
        <Text style={[st.hint, { marginTop: 10 }]}>Release to send</Text>
      </View>
    </View>
  );
}
...
const st = StyleSheet.create({
  ...
  bubble: { maxWidth: "84%", backgroundColor: tint(C.accent, 0.1), borderWidth: 1, borderColor: tint(C.accent, 0.22), borderRadius: 18, borderTopLeftRadius: 4, padding: 13 },
